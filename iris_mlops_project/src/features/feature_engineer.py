...
class FeatureEngineer:
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'FeatureEngineer':
        ...
        if self.create_polynomial:
            ...
            poly_feature_names = self.poly_transformer.get_feature_names_out(current_feature_names)
        ...
        if self.feature_selection:
            ...
            self.selected_feature_names = [
                name for name, selected in zip(current_feature_names, selected_mask)
                if selected
            ]
            logger.info(f"Selected features: {self.selected_feature_names}")

        self.final_feature_names = (
            self.selected_feature_names if self.feature_selection
            else current_feature_names
        )

        logger.info(f"Feature engineering fit complete. Final feature count: {len(self.final_feature_names)}")
        return self
