...
from utils.config import load_config, ProjectConfig
...
class MLPipeline:
    ...
    def setup_logger(self):
        """Set up logging."""
        setup_logger(
            log_file=f"logs/pipeline_{self.run_timestamp}.log",
            level="INFO"
        )
        self.logger = get_logger()
    ...
    def engineer_features(self) -> Dict[str, Any]:
        ...
        self.X_val_fe = self.feature_engineer.transform(X_val) if len(X_val) > 0 else X_val
        self.y_val = y_val
    ...
    def track_experiment(self):
        ...
        if self.config.experiment.log_models:
            self.experiment_tracker.log_model(
                self.trainer.model,
                f"model_{self.config.model.type}"
            )
    ...
    def register_model(self) -> str:
        ...
        if meets_threshold:
            self.model_registry.promote_to_staging(
                f"iris_{self.config.model.type}",
                version
            )
    ...
    def _print_final_summary(self):
        ...
        print(f"  - Features: {self.results['feature_engineering']['n_final_features']}")
        print(f"\nMetrics:")
        for metric, value in self.results['evaluation']['metrics'].items():
            print(f"  - {metric}: {value:.4f}")
        print(f"\nModel Version: {self.results['registration']['version']}")
        print(f"Meets Production Threshold: {self.results['registration']['meets_threshold']}")
        print("=" * 60 + "\n")
